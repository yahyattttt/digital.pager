...
- **Customer Pager UI**: Premium "Digital Pager" aesthetic with dark gradients, featuring a circular PagerDevice component with SVG LED animations and DSEG7 font for order numbers.
  ...
  - **Order Tracking Page**: Two completely separate flows based on `diningType`:
    - **Delivery orders** (`diningType === "delivery"`): Routed to `DeliveryTrackingView` — a clean, dedicated page. Shows "شكراً لطلبك" / "طلبك الآن في عهدة المندوب" header, live Firestore status card (بانتظار تأكيد المتجر → جاري التحضير → جاهز للتوصيل → تم التوصيل), prominent WhatsApp driver CTA, order number, and "إغلاق والعودة للرئيسية" session-close button. Rating popup auto-appears 1.5s after status reaches `ready`/`completed`/`archived`. No PagerDevice, no bell/sound/vibration, no share buttons. All audio suppressed in Firestore listener for delivery.
    - **Dine-in / Takeaway orders**: Standard Digital Pager flow unchanged — PagerDevice with LED animation, bell priming, sound alerts, share buttons, status transitions (`pending_verification` → `preparing` → `ready` → `archived`/`uncollected`/`rejected`). Rejected orders show bilingual "Order Rejected" screen.
