...
- **Sequential Order ID System**: Each store maintains separate counters for online (`onlineCounter`) and manual (`manualCounter`) orders in Firestore at `merchants/{uid}/settings/orderCounter`. Online orders get `ON-{n}` prefix (assigned on merchant acceptance), manual/pager orders get `MA-{n}` prefix (assigned on creation). `displayOrderId` field stored on both pager docs and whatsapp order docs. Legacy orders without `displayOrderId` fall back to `#orderNumber` display. Shift start and counter reset clear both counters. Dashboard search bar filters by `displayOrderId`. Tracking page shows `displayOrderId` on PagerDevice.
