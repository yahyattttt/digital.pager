...
- **Feature Control System**: Super Admin can toggle per-merchant access to sidebar modules via the Features dialog on any merchant card. Disabled modules show a professional lock placeholder instead of content — sidebar links remain visible. Controlled flags: `onlineOrdersEnabled`, `analyticsEnabled`, `trackingEnabled`, `crmEnabled`, `couponsEnabled`, `financialEnabled`, `reviewsEnabled`, `loyaltyModuleEnabled`, `loyaltyCrmEnabled`, `archiveEnabled`, `printReceiptsEnabled`, `smartRatingEnabled`. All default to `true`; stored as flat boolean fields on the merchant Firestore document. Endpoint: `PATCH /api/admin/merchant-features/:merchantId`.
...
- **Customer Tracking UI**: Clean order tracking interface at `/track/:orderId?m=:merchantId` with status-specific screens (awaiting, preparing, ready, completed, rejected, uncollected). Online orders show receipt buttons and share tracking links. Delivery orders use dedicated `DeliveryTrackingView`.
- **Manual Order Digital Pager**: Customer-facing pager at `/s/:storeId`. QR scan → order selection grid → confirmation dialog ("هل هذا هو رقم طلبك؟") → Phase 1 "Preparing" with share button and bell activation (0.5s audio unlock) → Phase 2 "Ready" with green screen and auto-play bell alert on Firestore `notified` status → Phase 3 "Rating" with 5-star system. Low ratings (1-3) show feedback text area, high ratings (4-5) show celebration message. Feedback saved as `customerFeedback` field on pager doc via `PATCH /api/pager-feedback`. Feedback visible to merchant in Order Archive expanded view. `GET /api/merchant-public/:merchantId` endpoint provides store info.
...
- **Order Tracking Page**: Distinct tracking flows for delivery orders (dedicated `DeliveryTrackingView` with live status updates and WhatsApp driver CTA) and dine-in/takeaway orders (clean status screens with order number display). Receipt button available across all non-rejected order statuses for online orders.
- **Digital Receipt System**: POS-style receipt page at `/receipt/:orderId?m=:merchantId` with white-on-black design, item table, delivery fee, VAT disclaimer, QR code (links back to receipt for verification), and print/PDF button. Server-side QR generation via `POST /api/receipt-qr` (max 500 chars).
- **Driver Control Link**: One-time-use `/driver-control/:orderId?m=:merchantId` page for delivery drivers. Shows two fat-finger-friendly buttons: "تم تسليم الطلب بنجاح" (marks `completed`) and "العميل لم يستلم الطلب" (marks `uncollected`). Kill-link mechanism: after action, page reloads and shows "link expired" state. Server validates status transitions (only from `ready/preparing/awaiting_confirmation/pending_verification`). Link is included in the WhatsApp "Share with Driver" message payload from the merchant dashboard.
- **Order Archive**: Comprehensive `/archive` dashboard view with searchable table of completed/cancelled/uncollected orders. Features: statistics header (revenue, count, avg), advanced filters (search, date range, type chips), CSV export, 20-per-page pagination, desktop table + mobile cards, per-order detail expansion, receipt view action. Sidebar entry with `FolderArchive` icon. Active/Archive tab toggle on overview. "View in Archive" link in order completion toast.
...
- **Manual Order System**: Allows merchants to manually enter 3-digit order IDs or use shift-based auto-generation, creating pagers isolated from WhatsApp order sync. CRITICAL: All pager creation now goes through server endpoints `POST /api/orders/pager` and `POST /api/orders/pager-auto` (uses `apikeyFetch` — bypasses Firestore client auth entirely). PIN toggle save uses `PATCH /api/merchant/:merchantId/pin-required`. These server routes are immune to client-side Firebase Auth expiry and Firestore rule changes.
