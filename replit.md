...
- **Order Entry System**: Two-mode system — Auto-Increment (Quick Add ⚡) creates orders with `lastOrderNumber + 1`, synced to Firestore at `merchants/{uid}/settings/orderCounter`. Manual mode allows typing any number. Shift Start dialog appears on first session if no counter exists. Settings page has Reset Counter (to #1 or custom number) and Shift Start button. All devices stay synced via onSnapshot on the counter document.
