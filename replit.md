...
- **Order Entry System**: Unified hybrid inline entry — an input field + Quick Add button side-by-side on both Overview and Waitlist views. Empty input → auto-increment (`lastOrderNumber + 1` via Firestore transaction); filled input → use that number AND snap counter to `max(current, manual)` to preserve monotonic ordering. Button text dynamically shows "Quick Add #N" or "Add #X" based on input. Input validated (positive integer only), cleared only on success. `handleSmartAdd(manualNumber?)` is the single handler. Dialog fallback available via FAB/empty-state. Shift Start dialog on first session if no counter exists. Settings page has Reset Counter and Shift Start. All devices synced via onSnapshot on `merchants/{uid}/settings/orderCounter`.
