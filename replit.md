...
- **Customer Pager UI**: Premium "Digital Pager" aesthetic with dark gradients, featuring a circular PagerDevice component with SVG LED animations and DSEG7 font for order numbers.
  ...
  - **Bell Notification System**: On the tracking page, the bell activation button is hidden during `pending_verification` and `preparing` status. When order transitions to `ready`, a pulsing button "الطلب صار جاهز ودك تفعل الجرس ؟" appears. Clicking plays `alert.mp3` directly (browser allows sound from user gesture). Button disappears after sound played or order moves past `ready`. No AudioContext priming, keep-alive, or background tab workarounds needed.
...
- **Structured Cloud Numbering System**: Separate Firestore counters (`onlineCounter`, `manualCounter`) per store. Online orders use `[CityCode][YY][PaddedCounter]` format (e.g., `0126001`), assigned at order creation on server via `generateOnlineOrderId()` Firestore REST transaction. Accept flow only updates status — does NOT regenerate IDs. Manual orders use `MA-{n}` via `manualCounter`. `cityCode` configurable in Settings (15 Saudi cities). `orderType` ("online"|"manual") field on orders/pagers. Counter auto-resets on year change.
