...
- **Order Tracking Page**: Distinct tracking flows for delivery orders (dedicated `DeliveryTrackingView` with live status updates and WhatsApp driver CTA) and dine-in/takeaway orders (standard Digital Pager flow with LED animation and bell priming). Receipt button available across all non-rejected order statuses for online orders.
