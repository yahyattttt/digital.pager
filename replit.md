...
- **Merchant Dashboard**: Professional SaaS dashboard with sidebar navigation for modules such as Dashboard, Online Section, Customer Feedback, Analytics, My Customers (CRM), Coupons, Financial, and Settings. Includes an Order Management Station for active orders and thermal receipt printing.
...
- **Delivery Feature**: Merchants can enable a delivery option with a configurable fee. Delivery orders include a Leaflet.js map picker for location, reverse geocoding via Nominatim API, and a "Share with Driver" WhatsApp integration with a pre-filled message including order details and a Google Maps link.
