...
- **Merchant Dashboard**: Professional SaaS dashboard with sidebar navigation (Dashboard, Waiting List, Online Section, Customer Feedback, Analytics, My Customers, Coupons, Settings).
  - **My Customers (عملائي)**: CRM table auto-populated from online orders. Columns: Name, Phone, Total Orders, Last Order Date. WhatsApp button per row opens pre-filled promotional message with coupon placeholder.
...
- **Direct Ordering System (Cash on Delivery)**: Merchants manage products in a "Digital Menu." Public menu displays products.
  ...
  - **Tracking Page**: Shows real-time order status flow: `pending_verification` → `preparing` → `ready` → `archived`.
    ...
    - **Preparing**: Digital Pager device with LED ring animation, "جاري التحضير" status. Shows "Enable Sound" button (red pulsing) for browser autoplay unlock.
    - **Ready**: Full pager alert with sound/vibration, "الطلب جاهز" / "ORDER READY!". If browser blocks audio, shows "Tap to hear alert" red fallback button. Any click/touch on page passively unlocks audio. `pendingAlertRef` tracks deferred alerts so any user interaction auto-triggers the bell.
