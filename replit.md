...
- **Customer Pager UI**: Premium "Digital Pager" aesthetic with dark gradients, featuring a circular PagerDevice component with SVG LED animations and DSEG7 font for order numbers.
  ...
  - **Delivery Address/Location**: When customer selects "delivery" diningType, a mandatory address text input + optional "Get My Current Location" geolocation button appears. Fields: `deliveryAddress` (required string), `deliveryLat`/`deliveryLng` (optional GPS coords). Saved to Firestore. Dashboard order cards show emerald delivery address box with clickable "Open in Google Maps" link (when coords available). Printed receipt includes delivery address. Tracking API returns delivery address fields. COD button disabled until address filled.
  - **Share with Driver (WhatsApp)**: Delivery orders show an emerald "المندوب 🟢" / "Driver 🟢" button on dashboard order cards (both pending and active). Clicking generates a formatted WhatsApp message with: order number, customer name/phone, items list with prices, delivery fee, total, address, Google Maps link (if coords), customer notes, and store name. If merchant has `driverPhone` saved in settings, wa.me links directly to that number; otherwise opens WhatsApp contact picker. Driver phone is configurable in Settings under delivery section.
