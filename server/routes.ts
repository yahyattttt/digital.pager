...
export async function registerRoutes(
  ...
  app.patch("/api/whatsapp-orders/:merchantId/:orderId", async (req, res) => {
    try {
      ...
      // ── Server-side loyalty reward crediting when order is archived ──
      if (status === "archived") {
        try {
          const orderDoc = await apikeyFetch(`${baseUrl}/merchants/${merchantId}/whatsappOrders/${orderId}`, { headers: {} });
          if (orderDoc.ok) {
            const od = await orderDoc.json();
            const of_ = od.fields || {};
            const loyaltyOptedIn = of_.loyalty_opted_in?.booleanValue === true;
            const rewardAlreadyApplied = of_.reward_applied?.booleanValue === true;
            const phone = of_.customerPhone?.stringValue || "";
            const orderTotal = parseFloat(String(of_.total?.doubleValue ?? of_.total?.integerValue ?? "0")) || 0;
            const sanitizedPhone = phone.replace(/\D/g, "");

            if (loyaltyOptedIn && !rewardAlreadyApplied && sanitizedPhone && orderTotal > 0) {
              const mRes = await apikeyFetch(`${baseUrl}/merchants/${merchantId}`, { headers: {} });
              if (mRes.ok) {
                const mDoc = await mRes.json();
                const lcFields = mDoc.fields?.loyalty_config?.mapValue?.fields || {};
                const isEnabled = lcFields.is_enabled?.booleanValue === true;
                const onlinePct = Number(lcFields.online_percent?.integerValue || lcFields.online_percent?.doubleValue || 0);

                if (isEnabled && onlinePct > 0) {
                  const earned = Math.round(orderTotal * onlinePct / 100 * 100) / 100;
                  if (earned > 0) {
                    const docId = `${merchantId}_${sanitizedPhone}`;
                    const wRes = await apikeyFetch(`${baseUrl}/wallets/${docId}`, { headers: {} });
                    let currentBalance = 0, currentOnlineBalance = 0, currentManualBalance = 0, currentVisits = 0, currentHistory: any[] = [];
                    if (wRes.ok) {
                      const wd = await wRes.json();
                      const wf = wd.fields || {};
                      currentBalance = parseFloat(String(wf.balance?.doubleValue ?? wf.balance?.integerValue ?? "0")) || 0;
                      currentOnlineBalance = parseFloat(String(wf.online_balance?.doubleValue ?? wf.online_balance?.integerValue ?? "0")) || 0;
                      currentManualBalance = parseFloat(String(wf.manual_balance?.doubleValue ?? wf.manual_balance?.integerValue ?? "0")) || 0;
                      currentVisits = parseInt(wf.visits_count?.integerValue || "0") || 0;
                      currentHistory = wf.history?.arrayValue?.values || [];
                    }
                    const newBalance = Math.round((currentBalance + earned) * 100) / 100;
                    const newOnlineBalance = Math.round((currentOnlineBalance + earned) * 100) / 100;
                    const histEntry = {
                      mapValue: {
                        fields: {
                          type: { stringValue: "earn_online" },
                          amount: { doubleValue: earned },
                          note: { stringValue: `مكافأة طلب أونلاين` },
                          timestamp: { stringValue: new Date().toISOString() },
                        },
                      },
                    };
                    const newHistory = [...currentHistory.slice(-49), histEntry];
                    const wFieldPaths = ["phone", "merchantId", "balance", "online_balance", "manual_balance", "visits_count", "history"];
                    const wMaskParams = wFieldPaths.map(fp => `updateMask.fieldPaths=${fp}`).join("&");
                    await apikeyFetch(`${baseUrl}/wallets/${docId}?${wMaskParams}`, {
                      method: "PATCH",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({
                        fields: {
                          ...
                          balance: { doubleValue: newBalance },
                          online_balance: { doubleValue: newOnlineBalance },
                          manual_balance: { doubleValue: currentManualBalance },
                          visits_count: { integerValue: String(currentVisits) },
                          history: { arrayValue: { values: newHistory } },
                        },
                      }),
                    });
                    // Mark order as reward applied + store amount for client display
                    await apikeyFetch(
                      `${baseUrl}/merchants/${merchantId}/whatsappOrders/${orderId}?updateMask.fieldPaths=reward_applied&updateMask.fieldPaths=reward_amount`,
                      {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                          fields: {
                            reward_applied: { booleanValue: true },
                            reward_amount: { doubleValue: earned },
                          },
                        }),
                      }
                    );
                    console.log(`[Loyalty] Online reward ${earned} SAR credited to ${sanitizedPhone} for order ${orderId} (${merchantId})`);
                  }
                }
              }
            }
          ...
          console.error("[Loyalty] Failed to credit online reward on archive:", loyaltyErr);
        }
      }
