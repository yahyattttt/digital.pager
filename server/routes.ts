...
export async function registerRoutes(
  ...
  app.get("/api/track/lookup", async (req, res) => {
    try {
      const { merchantId, orderNumber } = req.query;
      if (!merchantId || typeof merchantId !== "string" || !orderNumber || typeof orderNumber !== "string") {
        return res.status(400).json({ message: "merchantId and orderNumber query parameters required" });
      }
        ...
        structuredQuery: {
          ...
          where: {
            compositeFilter: {
              op: "AND",
              filters: [
                { fieldFilter: { field: { fieldPath: "orderNumber" }, op: "EQUAL", value: { stringValue: orderNumber } } },
                { fieldFilter: { field: { fieldPath: "status" }, op: "IN", value: { arrayValue: { values: [{ stringValue: "waiting" }, { stringValue: "notified" }] } } } },
              ],
            },
          },
          limit: 1,
        },
        parent: `projects/${projectId}/databases/(default)/documents/merchants/${merchantId}`,
      };
      ...
      if (!qRes.ok) return res.status(404).json({ message: "Order not found" });
      const results = await qRes.json();
      const docResult = results.find((r: any) => r.document);
      if (!docResult) return res.status(404).json({ message: "Order not found" });

      const docName = docResult.document.name;
      const docId = docName.split("/").pop();
      const f = docResult.document.fields || {};
      ...
      return res.json({ order, merchant, pagerId: docId });
    } catch (error) {
      console.error("Track lookup error:", error);
      return res.status(500).json({ message: "Failed to lookup order" });
    }
  });
